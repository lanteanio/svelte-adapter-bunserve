...
test('the cursor sub-budget itself refuses, not just the ceiling above it', async () => {
	// `cursor_lane` cannot say WHICH ceiling refused: a cursor upgrade the main
	// ceiling turned away carries the same label, because that is uws's label and
	// a cursor client cannot connect either way. So the reason appearing in the
	// test above is satisfied by a workload whose carve-out can never bind - a
	// `fraction` of 1 carves the whole ceiling, and every refusal would still
	// read `cursor_lane`. The controller's own count of sub-budget refusals is
	// the only thing that tells them apart.
	//
	// READ AFTER EACH RUN, with no before/after delta: every run resets the
	// controller on its way in, so a delta against the previous seed's final
	// value telescopes to "the last seed only" - which is how this assertion
	// first read zero over a workload that reaches the carve-out on half its
	// seeds.
	let subBudget = 0;
	for (const seed of ['1', '2', '3', '4', '5', '6', '7', '8']) {
		await runSeed(seed);
		subBudget += upgradeAdmission.cursorSubBudgetRefusals;
	}
	assert.ok(subBudget > 0, `the carve-out bound on some seed (saw ${subBudget})`);
});
